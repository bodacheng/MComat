...
public class RongLianGamen : MonoBehaviour {
    ...
    public void GUIRefresh() // 模块按理说应该是总体更新  
    {
        ...
        if (_preparingScene.currentProcess.step == MainSceneStep.JiNengRongLian_selectMaterialMonster)
        {
        ...
        else if (_preparingScene.currentProcess.step == MainSceneStep.JiNengRongLian_selectBaseMonster)
        {
            this._CameraManager.Assign_Camera(Camera_Mode_Num.LockCamera);
    ...
    IEnumerator RonglianAnimation()
    {
        ...
        {
            CharacterDataInfo baseMonster = AccountCharsSet.getTheCharacterOfMine(BaseCharLocalID);
            CharacterResourceInfo _CharacterResourceInfo = CharsManager._monstersConfigTable.RowToCharacterResourceInfo(
                CharsManager._monstersConfigTable.Find_ID(baseMonster.resource_num.ToString())
            );
    ...
    public void RongLianReturn() //这是一个直接在editor给返回按钮添加的函数。 
    {
        ...
        else if (_preparingScene.currentProcess.step == MainSceneStep.JiNengRongLian_selectBaseMonster)
        {
            ...
            {
                processingCharInfo = AccountCharsSet.getTheCharacterOfMine(_materialCharLocalID);
            }
            _preparingScene.trySwitchToStep(MainSceneStep.JiNengRongLian_selectMaterialMonster, true);
        }
        else if (_preparingScene.currentProcess.step == MainSceneStep.JiNengRongLian_waitForConfirm)
        {
            if (BaseCharLocalID != -1)
            {
                processingCharInfo = AccountCharsSet.getTheCharacterOfMine(BaseCharLocalID);
            }
    ...
    public void trySwitchToWaitForConfirm(IDictionary<int, GameObject> ModelDicBasedOnLocalID)
    {
        ...
        CharacterDataInfo materialMonster = AccountCharsSet.getTheCharacterOfMine(_materialCharLocalID);
        CharacterResourceInfo _CharacterResourceInfoofM = CharsManager._monstersConfigTable.RowToCharacterResourceInfo(
            CharsManager._monstersConfigTable.Find_ID(materialMonster.resource_num.ToString())
        );
