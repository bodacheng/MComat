    ...
    public void RoundBattleFieldNormalControl(Vector3 battleRingCenter, float BattleRingRadius)
    {
        ...
        foreach (KeyValuePair<Team, List<Data_Center>> pair in AllMembers)
        {
            foreach (Data_Center oneBoy in pair.Value)
            {
                ...
                {
                    battleRingCenter.y = oneBoy.gameObject.transform.position.y;
                    distanceFromCharToCenter = (oneBoy.gameObject.transform.position - battleRingCenter).magnitude;
                    if (distanceFromCharToCenter > BattleRingRadius)
                    {
                        oneBoy.onBattleGroundBundary = true;
                        oneBoy.gameObject.transform.position =
                                  Vector3.Lerp(oneBoy.gameObject.transform.position, battleRingCenter,
                                               Time.deltaTime);
                        oneBoy.antiWallDirection = battleRingCenter - oneBoy.gameObject.transform.position;
                    }
                    else
                    {
