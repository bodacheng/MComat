...
public partial class defaultPools
{
    ...
    public IEnumerator LoadAnimationPackFromCache(string url_withoutfilename,string dic_key, string packABName)
    {
        ...
        if (readingBundle != null)
        {
            Debug.Log("集合式动画包"+dic_key+"貌似提取成功");
    ...
    {
        IEnumerator task = DownloadAndCacheExactFile(Path,magicPackName);
        yield return task;
        yield break;
    }
    ...
    public IEnumerator LoadAnimationClipFromCachAndPutItIntoDic(string bundleURLwithoutclipname,string dic_key,string clip_name)
    {
        ...
        //yield return task;
        IEnumerator task = getABFromCach(bundleURLwithoutclipname,clip_name);
        yield return task;
        readingBundle = (AssetBundle)task.Current;
        ...
        if (readingBundle != null)
        {
            ...
            if (_AnimationClip != null)
            {
                ...
                if (!AnimationClipDic.ContainsKey(dic_key))
                    ...
                    AnimationClipDic[dic_key] = _AnimationClip;
    ...
    public IEnumerator LoadAudioClipFromCachAndPutItIntoDic(string bundleURL,string additionalPath, string clip_name)
    {
        ...
        IEnumerator task = getABFromCach(bundleURL,clip_name);
        yield return task;
        readingBundle = (AssetBundle)task.Current;
