...
public partial class UnitCreator
{
    public static IEnumerator CreateRawUnit_Resource(string monsterId)
    {
        ...
        var resultObject = Resources.Load("CharPretabs/" + _CharConfig.TYPE + "/" + _CharConfig.REAL_NAME) as GameObject;
        if (resultObject == null)
        {
            Debug.Log("资源错误："+"CharPretabs/" + _CharConfig.TYPE + "/" + _CharConfig.REAL_NAME);
            yield break;
        }
        ...
        _D.Zokusei = _CharConfig._zokusei;
        yield return (_D.Step1Initialize(_CharConfig.TYPE, _CharConfig.BASIC_MOVEMENT_PACK,_CharConfig.SPECIAL_ZOKUSEI));
        yield return _D;
    }
}
