...
public partial class BO_Marker_Manager : MonoBehaviour
{
    ...
    void OnEnable()//onenable函数是你把一个物体从对象池取出来后立刻就会执行，这样你需要仔细看这个函数和其他一些初始化相关的函数存不存在什么顺序错误
    {
        ...
        if (onEnableEffectT != null)
        {
            ...
            {
                processingBlood = defaultPools.Instance.GenerateEffect("on_enable_effect", this.personalEffectPath,
    ...
    //本企划下，原先的参数hitenergy也将删除，考虑到波动类技能的特点，不能让它在撞击到某把剑什么的时候也莫名其妙消失，所以“能量撞击”这个事情将由判断撞击到的武器的hp来决定。
    // 如果撞击到的武器的hp大于0，那这也说明了撞到的这个武器属于“能量系”的特点。
    void WeaponEnergyExaust(Vector3 Pos, Quaternion Qua)
    {
        if (weaponHP > 0)
            defaultPools.Instance.GenerateEffect("energy_resolve", this.personalEffectPath, Pos, Qua, null);
    ...
    {
        public hitOnHealthBody(BO_Health _BO_Health, Vector3 _Startpoint, Vector3 _Direction)
        {
        ...
        public Vector3 _Startpoint, _Direction;
    }
