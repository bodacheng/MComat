...
public partial class BO_Marker_Manager : MonoBehaviour {
    ...
    private void treatProcess()
    {
        if (HitShield && traditionalDefendMode)//其实由上面的分析可以知道，对于来自一把武器的攻击，hitshield和hitflesh是不会同时为true的。但如果多把武器同时来攻击，如果被攻击方同时有被击中以及防御住的情况发生，肯定要先处理所受伤害，立刻转入受伤状态才对
        {
            for (int i1 = 0; i1 < _Shields_Hit.Count; i1++)
            {
                if (!_Used_Targets.Contains(_Shields_Hit[i1])) //无论对墙壁，盾牌，还是伤害对象，每一轮攻击只会造成一次影响
                {
                    ...
                    {
                        this.myOwnerHealth.WhenIHitSomethingEnemy(1);
                        switch (collision.on_weapon_holder)
                        {
                            case damageType.stagger:
                                ...
                                new_damage = new v_Damage(0, damageType.stagger, force_direction, Vector3.zero, myOwnerHealth, this);
                    ...
                    if (TheS._ParentHealth != null)
                    {
                        switch (collision.on_shield_holder)
                        {
                            case damageType.light_block:
                                ...
                                new_damage = new v_Damage(0, damageType.light_block, force_direction, Vector3.zero, TheS._ParentHealth, this); //这地方是因为我们不了解往同一个列表里加入相同变量两次到底是啥结果。。。所以保险起见
                                TheS.plusHP(-1);
                            ...
                            case damageType.heavy_block:
                                ...
                                new_damage = new v_Damage(0, damageType.heavy_block, force_direction, Vector3.zero, TheS._ParentHealth, this); //这地方是因为我们不了解往同一个列表里加入相同变量两次到底是啥结果。。。所以保险起见
                                TheS.plusHP(-2);
                                TheS._ParentHealth.ApplyDamage(new_damage);
                                break;
                            case damageType.supper_damage:
                                force_direction = TheS._ParentHealth.transform.position - _WeaponHolderCenter.position;
                                new_damage = new v_Damage(0, damageType.supper_damage, force_direction, Vector3.zero, TheS._ParentHealth, this); //这地方是因为我们不了解往同一个列表里加入相同变量两次到底是啥结果。。。所以保险起见
                                TheS._ParentHealth.ApplyDamage(new_damage);
                                break;
                            case damageType.none:
                                break;
            ...
            if (_wallHitPositions.Count > 0)
            {
                ...
                for (int temp = 0; temp < _wallHitPositions.Count; temp++)
                {
                    ...
                    {
                        processingBlood = defaultPools.Instance.GenerateEffect("Sparks", this.personalEffectPath,
        ...
        {
            int hurt_hp;
            hurt_hp = (int)(Attack_And_Shield_Specification.Instance.AT_Damage_Magnification(damage_type) * basic_AT);
                ...
                if (_hitOnHealthBody._BO_Health != null && _Used_Targets.Contains(_hitOnHealthBody._BO_Health.transform) == false)
                {
                    ...
                    switch (_WeaponPosAdjustMode)
                    {
                        case WeaponPosAdjustMode.pushToMidForward:
                            ...
                            {
                                //平行四边形。用以攻击校准。
                                force_direction = myOwnerHealth.transform.forward * 1.5f - force_direction.normalized;
                        ...
                        case WeaponPosAdjustMode.draw:
                            force_direction = _hitOnHealthBody._Startpoint - _hitOnHealthBody._BO_Health.transform.position;
                    ...
                    //jiuzhengweizhi = this.transform.position;
                    //}
                    new_damage = new v_Damage(hurt_hp, damage_type, force_direction, _hitOnHealthBody._Startpoint, _hitOnHealthBody._BO_Health, this);
                    if(effectSpreadOnBody && _hitOnHealthBody._BO_Health.Resistance == 0)
                    ...
                    {
                        if (_hitOnHealthBody._BO_Health.Resistance > 0)
                        {
                            processingBlood = defaultPools.Instance.GenerateEffect("Sparks",
                                                                                   this.personalEffectPath,
                                                                                   _hitOnHealthBody._Startpoint,
                                                                                   Quaternion.LookRotation(_hitOnHealthBody._Direction, Vector3.up),
                                                                                   effectSpreadOnBody ? _hitOnHealthBody._BO_Health.getHealthBodyCenterTransform() : null);
                        }
                        else
                        {
                            switch (damage_type)
                            {
                                case damageType.slight_damage:
                                    processingBlood = defaultPools.Instance.GenerateEffect("light_hit",
                                                                                   this.personalEffectPath,
                                                                                   _hitOnHealthBody._Startpoint,
                                                                                   Quaternion.LookRotation(_hitOnHealthBody._Direction, Vector3.up),
                                                                                   effectSpreadOnBody ? _hitOnHealthBody._BO_Health.getHealthBodyCenterTransform() : null);
                                ...
                                case damageType.heavy_damage:
                                    processingBlood = defaultPools.Instance.GenerateEffect("heavy_hit",
                                                                                   this.personalEffectPath,
                                                                                   _hitOnHealthBody._Startpoint,
                                                                                   Quaternion.LookRotation(_hitOnHealthBody._Direction, Vector3.up),
                                                                                   effectSpreadOnBody ? _hitOnHealthBody._BO_Health.getHealthBodyCenterTransform() : null);
                                    ...
                                    break;
                                case damageType.supper_damage:
                                    processingBlood = defaultPools.Instance.GenerateEffect("super_hit",
                                                                                   this.personalEffectPath,
                                                                                   _hitOnHealthBody._Startpoint,
                                                                                   Quaternion.LookRotation(_hitOnHealthBody._Direction, Vector3.up),
                                                                                   effectSpreadOnBody ? _hitOnHealthBody._BO_Health.getHealthBodyCenterTransform() : null);
