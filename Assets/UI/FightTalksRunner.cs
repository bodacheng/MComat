    ...
    public void runBeforeFight(List<Data_Center> myTeamTs)
    {
        ...
        if (step < myTeamTs.Count)
        {
            if (timeCounter > 0.6f)
            {
                ...
                if (step < myTeamTs.Count)
                {
                    CountDown.text = myTeamTs[step]._CharacterDataInfo.resource_num.ToString();
                    _CameraManager.Assign_Camera(Camera_Mode_Num.WatchOverCamera, new List<Transform>() { myTeamTs[step].gameObject.transform });
                ...
                if (step == myTeamTs.Count)
                {
                    _CameraManager.Assign_Camera(Camera_Mode_Num.WatchOverCamera, allMuTs);
