...
public class SkillsPrintOut : MonoBehaviour {
    ...
    public Animation_Manger _FocusingAnimationManger;
    private AIStateRunner _FocusingAIStateRunner = new AIStateRunner();//这个只是说，我们要用里面的函数，它是不是哪个角色身上的那个runner无所谓。
    ...
    {
        if (_FocusingAnimationManger != null && _FocusingAnimationManger.Animator != null && _FocusingAnimationManger.gameObject.activeSelf)
        {
            if (_FocusingAnimationManger.GetAnimationPlayingStep() == AnimationPlaying_Step.over)
            {
                this._FocusingAnimationManger.PlayLayerAnim(animator_layer_index.Full_Body, null);
                _FocusingAnimationManger.setAnimationPlayingStep(AnimationPlaying_Step.unstarted);
                this._CameraManager.Assign_Camera(Camera_Mode_Num.LockCamera);
                this._CameraManager.current_Camera_Mode.targets = new List<Transform>() { MemDetailWatchPos};//通常情况下，相机是所在固定位置上。模型主动定在屏幕左边
                _FocusingAnimationManger.gameObject.transform.localPosition = Vector3.zero;
            }
        }
    }
    ...
    void addShowSkillInfoFeature(Button _button, State_Transition_Set _state_Transition_Set)
	{
        ...
        UnityEngine.Events.UnityAction showSkillInfo = () =>
        {
            ...
            _CameraManager.drawlines(_toDrawLines);
            ...
            if (this._FocusingAnimationManger != null)
            {
                ...
                this._FocusingAnimationManger.animationCustomCoroutineTrigger(animator_layer_index.Full_Body, _state_Transition_Set.StateKey);
            }
            else{
    ...
    {
        CharacterResourceInfo _watchingCharacterResourceInfo = CharsManager.getCharacterResourceInfo(_watchingCharInfo.resource_num);
    ...
    public void sKillScriptReader(string type, NineAndTwo nineAndTwo, passiveSkillConfigs passiveSkillConfigs,int AI_level)
	{
        ...
        this._FocusingAIStateRunner.FormFightingSetsByNineAndTwo(type, nineAndTwo,passiveSkillConfigs, AI_level);
        analysisStatesList.Clear();
        analysisStatesList = this._FocusingAIStateRunner.State_Transition_Set_List;
        analysisStatesSetDic.Clear();
        ...
        _toDrawLines = new List<Vector3[]>();
        _CameraManager.ClearDrawingLines();
        ...
        for (int i = 1; i < 4; i++)
        {
            ...
            if (Fire1_chuan[i].SPLevel == EX.EX1)
            {
                newShow = Instantiate(ex1showbutton);
                newShow.GetComponentInChildren<Image>().color = Color.blue;
            }
			else if (Fire1_chuan[i].SPLevel == EX.EX2)
            {
                newShow = Instantiate(ex2showbutton);
                newShow.GetComponentInChildren<Image>().color = Color.red;
            }
			else if (Fire1_chuan[i].SPLevel == EX.EX3)
            {
                newShow = Instantiate(ex3showbutton);
                newShow.GetComponentInChildren<Image>().color = Color.white;
            }
        ...
        for (int i = 1; i < 4; i++)
        {
            ...
            if (Fire2_chuan[i].SPLevel == EX.EX1)
            {
                newShow = Instantiate(ex1showbutton);
                newShow.GetComponentInChildren<Image>().color = Color.blue;
            }
			else if (Fire2_chuan[i].SPLevel == EX.EX2)
            {
                newShow = Instantiate(ex2showbutton);
                newShow.GetComponentInChildren<Image>().color = Color.red;
            }
			else if (Fire2_chuan[i].SPLevel == EX.EX3)
            {
                newShow = Instantiate(ex3showbutton);
                newShow.GetComponentInChildren<Image>().color = Color.white;
            }
