...
public static class AIScriptReading {
    ...
    public static List<SkillEntity> ReadKongfuBook(BehaviorRunner _AIStateRunner,TextAsset Script)
    {
        try
        {
            ...
            if (list == null)
            {
                list = new List<SkillEntity>() {new SkillEntity("Empty",0,0,0,0,0,null ,null,InputKey.Null, InputKey.Null,0,0)};
            }else{
                if (list.Count == 0)
                {
                    list.Add(new SkillEntity("Empty",0,0,0,0,0,null,null,InputKey.Null, InputKey.Null,0,0));
                }
            }
            return list;
        }
        catch (NullReferenceException e)
        {
            ...
            return new List<SkillEntity>() {new SkillEntity("Empty",0,0,0,0,0,null,null,InputKey.Null, InputKey.Null,0,0)};
        }
    }
}
