...
public abstract class AI_State
{
    ...
    public AI_DATA_CENTER AI_DATA_CENTER;
    ...
    public customCoroutine _customCoroutine;

    public string StateKey;
    public EX splevel;
    public bool playerModeInputDepend = true;
    ...
    public virtual void pre_process_before_enter() {
		this.AI_DATA_CENTER = gameObject.GetComponent<AI_DATA_CENTER>();
		this.BS_Main_Health = gameObject.GetComponent<BO_Health>();
        this.shaderManager = BS_Main_Health._ShaderManager;
        this.Sensor = AI_DATA_CENTER.Sensor;
        this._AIStateRunner = gameObject.GetComponent<AIStateRunner>();
        this._inputManager = this._AIStateRunner._inputManager;
		this._Animator = gameObject.GetComponent<Animator>();
		this._SkillCancelFlag = gameObject.GetComponent<SkillCancelFlag>();
        this._BO_Ani_E = gameObject.GetComponent<BO_Ani_E>();
        this._Weapon_Animation_Events = gameObject.GetComponent<BO_Weapon_Animation_Events>();
    ...
    public virtual void _c_State_Update() {
        this._f_State_Update();
    }

    // Local update of the state 
    public virtual void _f_State_Update() {

    }

    // Local fixedupdate of the state 
    public virtual void _State_FixedUpdate() {

    }

    // Local fixedupdate of the state 
    public virtual void _c_State_FixedUpdate() {
        this._State_FixedUpdate();
    }
    ...
    protected float RotateToTarget(Vector3 target, float turnSpeed, bool ignoreY) {
        ...
        gameObject.transform.rotation = Quaternion.Slerp(gameObject.transform.rotation, dirQ, turnSpeed * Quaternion.Angle(dirQ, gameObject.transform.rotation) * Time.deltaTime);
