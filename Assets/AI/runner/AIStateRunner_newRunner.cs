...
public partial class AIStateRunner : MonoBehaviour
{
    ...
    public void stateTransitionEngine_new(IDictionary<string, State_Transition_Set> state_Transition_Dictionary)
    {
        ...
        if (avaliable_casual_Transitions.Count > 0)
        {
            if (_focusing == this && (this.playerMode || _inputManager.ifPlayerIsInputting()))
            {
                _inputManager.SkillButtonPressedExplode(avaliable_casual_Transitions[0].enterInput,avaliable_casual_Transitions[0].SPLevel);
    ...
    private void randomTransitionToRun(List<State_Rate_Set> TransitionsToRun)
    {
        ...
        if (_focusing == this)
            _inputManager.SkillButtonPressedExplode(TransitionsToRun[next].enterInput,TransitionsToRun[next].SPLevel);
        _SkillCancelFlag.turn_off_flag();
        ...
        if (AINext.Count > 0)
        {
            if (_focusing == this && (this.playerMode || _inputManager.ifPlayerIsInputting()))
            {
                foreach (AI_State State_Rate_Set in AINext)
                {
                    ...
                    {
                        _inputManager.SkillButtonPressedExplode(State_Rate_Set.enterInput,State_Rate_Set.splevel);
