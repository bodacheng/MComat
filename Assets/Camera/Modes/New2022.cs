...
class New2022 : CameraMode//  改编自CertainYAntiVabration
{
    ...
    public override void LocalUpdate(Camera _camera)
    {
        ...
        if (targets != null && targets.Count > 0)
        {
            ...
            var rate = Vector2.Angle((backScreenPos - frontScreenPos), Vector2.right) / 180;
            ...
            cameraTargetPos = (backWPos - frontWPos) * rate / 2 + frontWPos + xzOff.normalized * XZDistance;
            cameraTargetPos += Vector3.up * YDis;
            fixY = Mathf.Clamp(cameraTargetPos.y, YDis, cameraTargetPos.y);
            cameraTargetPos.y = fixY;
            
            lookPoint = (backWPos - frontWPos) * (1 - rate) / 2 + frontWPos;
            lookPoint.y = 2.3f;
