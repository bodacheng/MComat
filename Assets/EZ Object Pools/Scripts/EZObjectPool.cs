...
namespace EZObjectPools
{
	...
	public class EZObjectPool : MonoBehaviour
    {
        ...
        static bool PoolHasFlawedMember = false;
        static List<GameObject> flawedMember = new List<GameObject>();
        public static void ReparentPooledObjects(EZObjectPool Pool,bool AllorJustInactiveones)
        {
            PoolHasFlawedMember = false;
            flawedMember.Clear();
            foreach (GameObject _gameObject in Pool.ObjectList)
            {
                if (_gameObject != null)
                {
                    if ((!_gameObject.activeSelf || AllorJustInactiveones) && _gameObject.transform.parent != Pool.transform)
                    {
                        _gameObject.transform.parent = Pool.transform;
                        //Pool.AddToAvailableObjects(_gameObject);
                        if (!Pool.AvailableObjects.Contains(_gameObject))
                            Pool.AvailableObjects.Add(_gameObject);
                    }
                }else{
                    PoolHasFlawedMember = true;
                    flawedMember.Add(_gameObject);
                    Debug.Log(Pool.name + "对象池貌似出现了一些清理上的问题，强行进行排除处理。。");
                }
            }
            if (PoolHasFlawedMember)
            {
                for (int i = 0; i < flawedMember.Count; i++)
                {
                    Pool.AvailableObjects.Remove(flawedMember[i]);
                    Pool.ObjectList.Remove(flawedMember[i]);
                }
            }
        }
        ...
        void Awake()
        {
            ...
            if (InstantiateOnAwake)
            {
                ...
                this.AvailableObjects = new List<GameObject>(PoolSize);
                InstantiatePool();
            }
        ...
        public void SetProperties(GameObject objectTemplate, int size, string name, bool autoResize)
        {
            ...
            PoolSize = size;
            this.ObjectList = new List<GameObject>(size);
            this.AvailableObjects = new List<GameObject>(size);
            PoolName = name;
		...
		public bool TryGetNextObject(Vector3 pos, Quaternion rot, out GameObject obj)
        {
            ...
            int lastIndex = this.AvailableObjects.Count - 1;
            ...
            if (this.AvailableObjects.Count > 0)
            {
                ...
                this.AvailableObjects[lastIndex].SetActive(true);
                this.AvailableObjects[lastIndex].transform.position = pos;
                this.AvailableObjects[lastIndex].transform.rotation = rot;
                obj = this.AvailableObjects[lastIndex];
                this.AvailableObjects.RemoveAt(lastIndex);
                return true;
            }
		...
		public GameObject TryGetNextObject(Vector3 pos, Quaternion rot)
        {
            if (ObjectList.Count == 0)
            {
                Debug.Log("EZ Object Pool " + PoolName + ", the pool has not been instantiated but you are trying to retrieve an object!");
                Debug.Log("对象池内成员数量为0，尝试重新初始化成员。。");
            ...
            if (this.AvailableObjects.Count > 0)
            {
                if (this.AvailableObjects[lastIndex] == null)
                {
                    Debug.LogError("EZObjectPool " + PoolName + " has missing objects in its pool! Are you accidentally destroying any GameObjects retrieved from the pool?");
                    return null;
                }

                if (!this.AvailableObjects[lastIndex].activeSelf)
                {
                    this.AvailableObjects[lastIndex].SetActive(true);
                    this.AvailableObjects[lastIndex].transform.position = pos;
                    this.AvailableObjects[lastIndex].transform.rotation = rot;
                    GameObject g = this.AvailableObjects[lastIndex];
                    this.AvailableObjects.RemoveAt(lastIndex);
                    return g;
                }
            }
        ...
        {
            if (!this.AvailableObjects.Contains(obj))
        ...
        {
            return ObjectList.Count - this.AvailableObjects.Count;
        }
